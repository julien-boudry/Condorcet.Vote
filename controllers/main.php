...
abstract class Controller
{
    ...
    {
        if (!\is_array(self::$_head_JS)) {
            self::$_head_JS = [];
        }

        for ($i = $priority * 100; isset(self::$_head_JS[$i]); $i++) {
        }

        self::$_head_JS[$i] = ['url' => $url, 'integrity' => $integrity, 'crossorigin' => $crossorigin];
    }

    protected static function AddCSS($url, $priority = 0, $integrity = false, $crossorigin = false): void
    {
        for ($i = $priority * 100; isset(self::$_head_CSS[$i]); $i++) {
        }
        self::$_head_CSS[$i] = ['url' => $url, 'integrity' => $integrity, 'crossorigin' => $crossorigin];
    }
    ...
    public function showPage(?self $follow = null, string $position = 'after')
    {
